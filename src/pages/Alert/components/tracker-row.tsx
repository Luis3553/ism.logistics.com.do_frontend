...
import { TrackerGroup } from "@utils/types";

export function TrackerRow({ tracker }: { tracker: TrackerGroup }) {
    const { width } = useWindowSize();
    const [open, setOpen] = useState(true);
    const groupRows = useMemo(() => {
        return tracker.alerts.map((alert, alertIdx) => <NotificationRow key={`alert-${alertIdx}`} notification={alert} width={width!} type={"alert"} />);
    }, [tracker.alerts, width]);
